...
'''
"""Record Franka arm-only demos with SoFranka subarm teleoperation."""
...
DEFAULT_FPS = 15
DEFAULT_TASK_DESCRIPTION = "Teleoperate Franka arm with subarm."
DEFAULT_DATASET_NAME = "franka_fer_subarm"
...
def show_preview_if_available(observation: dict):
    ...
    try:
        ...
        cv2.imshow(f"franka_fer_subarm_record preview: {cam_name}", bgr)
        cv2.waitKey(1)
    except Exception:
        return
...
def main():
    ...
    logger.info("Setting up Franka arm-only subarm recording...")
    ...
    try:
        ...
        _init_rerun(session_name="franka_fer_subarm_record")
