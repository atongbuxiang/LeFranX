...
void close_socket(socket_handle_t& socket_handle) {
...
std::string socket_address_to_string(const sockaddr_in& address) {
...
private:
    // TCP connection
    socket_handle_t tcp_socket_ = kInvalidSocket;
    socket_handle_t client_socket_ = kInvalidSocket;
...
public:
    ...
    bool start_tcp_server() {
        ...
        if (!initialize_socket_api()) {
            std::cerr << "Failed to initialize socket API" << std::endl;
            return false;
        }
    ...
    py::dict get_status() {
        ...
        py::dict status;
        status["tcp_connected"] = (client_socket_ != kInvalidSocket);
        ...
        while (running_) {
            try {
                ...
                if (client_socket_ == kInvalidSocket) {
                    ...
                    socket_len_t client_len = sizeof(client_address);
                    ...
                    if (config_.verbose) {
                        std::cout << "VR client connected from " << socket_address_to_string(client_address) << std::endl;
                    }
                }
